									...
									<div class="col-md-8 col-xs-8">
										<div class="postListInfo">
											...
											<c:if test="${postListDto.review_writable eq 0 and info.post_status eq 'S'}">
												<input type="button" value="리뷰쓰기" class="btn btn-default" onclick="location='/gonggu/review/write?payment_id=${postListDto.payment_id}'">
											</c:if>								
										</div>							
									</div>
								</div>
						...
						<div class="col-md-2 col-md-offset-5 hf font-DH text-center mar-top-20">
							...
							<c:if test="${info.cnt gt 0}">
								...
								<c:if test="${info.startPage gt info.pageBlock}"> 
									<a href="/gonggu/mypage/posts/payed?pageNum=${info.startPage-info.pageBlock}">◀</a>
								</c:if>
								</div>
								<div class="col-md-8">
								<c:forEach var="i" begin="${info.startPage}" end="${info.endPage}">
									...
									<c:if test="${i ne info.currentPage}">
										<a href="/gonggu/mypage/posts/payed?pageNum=${i}">[${i}]</a>
									</c:if>
								</c:forEach>
								</div>
								<div class="col-md-2">
